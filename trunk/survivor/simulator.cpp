 ...
 */
bool simulator::parseFile(char * filename)
{
    /* open the file */
...
infested_area * simulator::parseArea(string line)
{
    ...
    tokenizer tok(line, simulator::TOKENS);
    while(tok.has_next())
    {
        tok_count++;
...
survivor * simulator::parseSurvivor(string line)
{
        /* tokenize each line and create and store relevant objects */
